...
export var AllDatastoreEntryTypes: DatastoreEntryType[] = [
...
export class DatastoreEntry {
   ...
   */
  static from_server_def(
    entry_type: DatastoreEntryType,
    data: API.WatchableEntryServerDefinition
  ): DatastoreEntry {
    ...
    const entry = new DatastoreEntry(
      entry_type,
      data["id"],
      data["display_path"],
      data["datatype"],
  ...
  set_value(val: number): void {
    this.value = val;
    Object.keys(this.callbacks).forEach((watcher) => {
      this.callbacks[watcher].forEach(function (cb) {
        cb(val);
      });
    });
  }
...
type DatastoreTreesType = Record<
  DatastoreEntryType,
  Tree<DatastoreEntryWithName>
>;
...
export class Datastore {
  ...
  clear(entry_types?: DatastoreEntryType[]) {
    ...
    for (let i = 0; i < entry_types.length; i++) {
      ...
      this.trigger("scrutiny.datastore.clear", {
   ...
   * @return a callback to call to stop watching
   */
  watch(
    entry_type: DatastoreEntryType,
    entry: DatastoreEntry | string,
    watcher: string,
    ...
    if (!this.watched_entries.has(entry)) {
      ...
      this.trigger("scrutiny.datastore.start_watching", {
        entry: entry,
      });
    }
  }
  ...
  unwatch_all(watcher: string): void {
    if (this.watcher2entry.hasOwnProperty(watcher)) {
      this.watcher2entry[watcher].forEach((entry) => {
        entry.unwatch(watcher);
        if (!entry.has_watchers()) {
          this.trigger("scrutiny.datastore.stop_watching", {
            entry: entry,
          });
          this.watched_entries.delete(entry);
        }
      });
  ...
  add(entry: DatastoreEntry): void {
    ...
    if (this.serverid2entry.hasOwnProperty(entry.server_id)) {
      throw new Error("Duplicate server ID in datastore" + entry.server_id);
    }
  ...
  add_from_server_def(
    entry_type: DatastoreEntryType,
    def: API.WatchableEntryServerDefinition
  ): void {
  ...
  get_entry_from_server_id(server_id: string): DatastoreEntry {
    if (!this.serverid2entry.hasOwnProperty(server_id)) {
      throw new Error("No entry with server ID " + server_id + " in datastore");
    }
  ...
  set_value(
    entry_type: DatastoreEntryType,
    entry_path: string | DatastoreEntry,
    val: number
  ): void {
   ...
   */
  get_count(entry_type: DatastoreEntryType): number;
  get_count(): Record<DatastoreEntryType, number>;
  get_count(
    entry_type?: DatastoreEntryType
  ): number | Record<DatastoreEntryType, number> {
    if (typeof entry_type == "undefined") {
      let obj_out = {} as Record<DatastoreEntryType, number>;
      for (let i = 0; i < AllDatastoreEntryTypes.length; i++) {
        obj_out[AllDatastoreEntryTypes[i]] =
          this.trees[AllDatastoreEntryTypes[i]].count();
      }
      return obj_out;
    } else {
  ...
  set_ready(entry_type: DatastoreEntryType): void {
    if (this.ready[entry_type] === false) {
      this.trigger("scrutiny.datastore.ready", {
        entry_type: entry_type,
      });
    }
  ...
  get_children(
    ...
    let tree_children: ShallowNodeDescription<DatastoreEntryWithName> | null =
      null;
    try {
    ...
    for (let i = 0; i < node_names.length; i++) {
      ...
      named_entry["default_name"] = trim(named_entry.display_path, "/")
        .split("/")
        .pop();
