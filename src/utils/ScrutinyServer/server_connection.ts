...
class LoadWatchableSession {
   ...
   */
  set_expected_datastore_size(
    sizes_per_types: Record<DatastoreEntryType, number>
  ): void {
    if (this.download_type === WatchableDownloadType.RPV) {
      ...
      if (
        !sizes_per_types.hasOwnProperty(DatastoreEntryType.Var) ||
        !sizes_per_types.hasOwnProperty(DatastoreEntryType.Alias)
      ) {
        this.cancel();
    ...
    for (let i = 0; i < AllDatastoreEntryTypes.length; i++) {
      if (sizes_per_types.hasOwnProperty(AllDatastoreEntryTypes[i])) {
        this.expected_datastore_size[AllDatastoreEntryTypes[i]] =
          sizes_per_types[AllDatastoreEntryTypes[i]];
      }
    }
  }
  ...
  get_required_datastore_size(entry_type: DatastoreEntryType): number | null {
    if (
      entry_type === DatastoreEntryType.RPV &&
      !(this.download_type === WatchableDownloadType.RPV)
    ) {
      throw new Error('Entry type "RPV" not expected for this download');
    }
    if (
      entry_type === DatastoreEntryType.Alias &&
      !(this.download_type === WatchableDownloadType.Var_Alias)
    ) {
      throw new Error('Entry type "Alias" not expected for this download');
    }

    if (
      entry_type === DatastoreEntryType.Var &&
      !(this.download_type === WatchableDownloadType.Var_Alias)
    ) {
...
export class ServerConnection {
  ...
  active_download_session: Record<
    WatchableDownloadType,
    LoadWatchableSession | null
  >;
  ...
  protected registerToEvents() {
    this.register_api_callback(
      "inform_server_status",
      (data: API.Message.S2C.InformServerStatus) => {
        this.inform_server_status_callback(data);
      }
    );

    this.register_api_callback(
      "response_get_watchable_list",
      (data: API.Message.S2C.GetWatchableList) => {
    ...
    this.register_api_callback(
      "watchable_update",
      (data: API.Message.S2C.WatchableUpdate) => {
    ...
    if (this.socket !== null) {
      if (this.socket.readyState === this.socket.OPEN) {
        try {
          ...
          reqid = null;
          this.logger.error(
            "Cannot send request with command=" + cmd + ". Error: " + e
          );
        }
      }
    }
  ...
  on_socket_message_callback(msg: string): void {
    try {
      ...
      if (obj.cmd === "error") {
        let error_message =
          'Got an error response from the server for request "' +
          obj.request_cmd +
          '".';
  ...
  receive_watchable_list(data: API.Message.S2C.GetWatchableList): void {
    ...
    const var_alias_session =
      this.active_download_session[WatchableDownloadType.Var_Alias];
    ...
    try {
      if (this.server_status !== ServerStatus.Connected) {
        ...
        if (download_type === WatchableDownloadType.Var_Alias) {
          const required_size_var =
            download_session.get_required_datastore_size(
              DatastoreEntryType.Var
            );
          const required_size_alias =
            download_session.get_required_datastore_size(
              DatastoreEntryType.Alias
            );
          ...
          if (required_size_var == null || required_size_alias == null) {
            ...
            for (let i = 0; i < data["content"]["var"].length; i++) {
              this.datastore.add_from_server_def(
                DatastoreEntryType.Var,
                data["content"]["var"][i]
              );
            }
            ...
            if (
              ...
              download_session.cancel();
              this.logger.error(
                "Server gave more data than expected. Download type = " +
                  download_type
              );
            }
          }
        ...
        if (download_type === WatchableDownloadType.RPV) {
          const required_size_rpv =
            download_session.get_required_datastore_size(
              DatastoreEntryType.RPV
            );
  ...
  inform_server_status_callback(data: API.Message.S2C.InformServerStatus) {
    const device_status_str_to_enum: Record<
      API.ServerDeviceStatus,
      DeviceStatus
    > = {
    ...
    try {
      ...
      try {
        ...
        if (data["loaded_sfd"] == null && this.loaded_sfd != null) {
          ...
          if (this.loaded_sfd == null) {
            ...
            this.loaded_sfd["firmware_id"] !== data["loaded_sfd"]["firmware_id"]
          ) {
            must_reload = true;
          }
        ...
        this.loaded_sfd = null;
        this.logger.error(
          "[inform_server_status] Cannot read loaded firmware. ",
          e
        );
      }
      ...
      try {
        ...
        this.device_info = null;
        this.logger.error(
          "[inform_server_status] Cannot read device info. ",
          e
        );
      }
    } catch (e) {
